...
import {
  ...
  MdOutlineNotificationsNone, MdOutlineSettings, MdBook, MdSelfImprovement, MdPeople, MdPerson, MdEdit, MdVisibility
} from "react-icons/md";
import { FaGamepad, FaHeart, FaHandshake, FaUmbrellaBeach } from "react-icons/fa";
import { useNavigate } from 'react-router-dom';
import '../HomePage.css';
import './ConnectionDiscoverySuite.css';
import RichProfilesMatching from './RichProfilesMatching';
import SeeMyMatches from './SeeMyMatches';
import RichProfileView from './RichProfileView';
import MatchSuccessScreen from './MatchSuccessScreen';

const ConnectionDiscoverySuite: React.FC = () => {
  const [activeSidebar, setActiveSidebar] = useState("discovery");
  const [showDropdown, setShowDropdown] = useState(false);
  const [activeTab, setActiveTab] = useState<'matching' | 'matches' | 'profile' | 'match-success'>('matching');
  ...
  return (
    <div className="home-root" style={{ minHeight: '100vh', height: '100%', background: 'linear-gradient(135deg, #23235b 0%, #2a1a4a 100%)' }}>
      ...
      <main className="main-content with-topbar-margin" style={{ width: '100%' }}>
        ...
        {/* Connection & Discovery Suite Header */}
        <section className="discovery-header">
          <h1 className="discovery-title">Connection & Discovery Suite</h1>
          <p className="discovery-subtitle">Find meaningful connections through rich profiles and intelligent matching</p>
        </section>

        {/* Navigation Tabs */}
        <div className="discovery-tabs">
          <button
            className={`discovery-tab ${activeTab === 'matching' ? 'active' : ''}`}
            onClick={() => setActiveTab('matching')}
          >
            <MdPeople /> Rich Profiles & Matching
          </button>
          <button
            className={`discovery-tab ${activeTab === 'matches' ? 'active' : ''}`}
            onClick={() => setActiveTab('matches')}
          >
            <MdFavorite /> See My Matches
          </button>
          <button
            className={`discovery-tab ${activeTab === 'profile' ? 'active' : ''}`}
            onClick={() => setActiveTab('profile')}
          >
            <MdPerson /> My Profile
          </button>
          <button
            className={`discovery-tab ${activeTab === 'match-success' ? 'active' : ''}`}
            onClick={() => setActiveTab('match-success')}
          >
            <FaHeart /> Match Success
          </button>
        </div>

        {/* Content Area */}
        <div className="discovery-content">
          {activeTab === 'matching' && <RichProfilesMatching />}
          {activeTab === 'matches' && <SeeMyMatches />}
          {activeTab === 'profile' && <RichProfileView />}
          {activeTab === 'match-success' && <MatchSuccessScreen />}
        </div>
      </main>
    </div>
  );
...
export default ConnectionDiscoverySuite;
